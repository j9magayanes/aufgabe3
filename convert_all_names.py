def remove_prefix(name):
    ...
    elif name.startswith("item.Process."):
        return name[len("item.Process."):]
    elif name.startswith("item.Result."):
...
def process_data(data):
    modified_data = []
    for item in data:
        modified_item = {"Name": item["Name"], "Variables": []}
        for var in item["Variables"]:
            modified_var_name = remove_prefix(var["Name"])
            modified_var = {"Name": modified_var_name, "Type": var["Type"]}
            if "Value" in var:
                modified_var["Value"] = var["Value"]
            modified_item["Variables"].append(modified_var)
